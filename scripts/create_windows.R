...
data.frame(unlist(regions)) %>% rename(start = "unlist.regions.") %>% mutate(CHROM = paste(chromosome), end = start+1000) %>% select(CHROM, start, end) %>% write.table(args[4], sep = "\t", quote = F, col.names = F, row.names = F)
